---
id: "char-cornelis-2026-06-28"
status: "todo"
priority: "high"
assignee: null
epic: "characters"
dueDate: "2026-07-31"
created: "2026-06-28T12:00:00.000Z"
modified: "2026-06-28T12:00:00.000Z"
completedAt: null
labels: ["character", "villager", "world"]
order: "chr-3"
---
# Character: Cornelis (villager)

**Role:** Villager | **First appears:** I.2 | **Location:** Moat house

Fussy, self-important. Convinced the moat makes him the safest man in Beaverford. Shouts from his window. Stationary NPC — always at or near his window.

**Build tasks:**
1. Assemble `CharacterAgent2D` node with Cornelis's sprites/model.
2. Configure `CharacterData` resource.
3. Animation: idle (window-shouting pose), no movement needed.
4. Set up dialogue nodes for I.2, II.1, II.2, III.2/III.3 (Ghost + Night Shift cycles).
